...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return {
    title: service ? `${service.title} | Cheema Advertising` : "Signboard | Cheema Advertising",
    ...
    <main className="min-h-screen bg-white">
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-white">
          <p className="text-sm uppercase tracking-[0.2em] text-white/80">Signboards</p>
          <h1 className="text-4xl md:text-5xl font-bold mt-2 mb-4">{service.title}</h1>
          <p className="text-lg max-w-3xl text-white/90">{service.description}</p>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-10 items-start">
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            <h3 className="text-xl font-semibold text-gray-900 mb-3">Need this service?</h3>
            <p className="text-gray-600 mb-4">Tell us about your project and we will respond quickly.</p>
            <Link href="/contact" className="inline-flex items-center bg-primary text-black px-5 py-3 rounded-lg font-semibold hover:bg-primary/90 transition">
              Contact Us
            </Link>
          </div>
        </div>
